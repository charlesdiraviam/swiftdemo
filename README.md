...
.github/workflows/azure-static-web-apps-*.yml   — GH Actions → SWA deploy
                 (generated by Azure; deploys the repo root)
...
There is no build step — the repo root *is* the deployed site. Confirm the two
...
#    The Azure-generated workflow triggers on push to main.
```
...
- A workflow file at `.github/workflows/azure-static-web-apps-<name>.yml`.
  **This is the deploy path — leave it in place.** Note the secret it
  references is named after the SWA resource (e.g.
  `AZURE_STATIC_WEB_APPS_API_TOKEN_AMBITIOUS_STONE_055862900`), not the bare
  `AZURE_STATIC_WEB_APPS_API_TOKEN`. A second workflow referencing the
  unsuffixed name will fail with `deployment_token was not provided`.
...
2. Uploads the repo root (`app_location: "/"`) to the SWA edge.
3. Posts the preview URL as a PR comment, and tears the preview down on close.
...
| `deployment_token was not provided`           | The workflow references a secret name that doesn't exist. Run `gh secret list` and use the exact suffixed name Azure created. |
...
| Site loads but scripts 404                   | Confirm `mock/` is committed and `app_location: "/"` is in the YAML.        |
| `404 — index.html not found`                  | `index.html` must be at the repo root, matching `app_location: "/"`.        |
